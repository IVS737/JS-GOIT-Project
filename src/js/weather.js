...
const fetchWeatherCity = async (cityName = 'Kyiv', units = 'metric') => {
  const { data } = await axios.get(`${URL}?q=${cityName}&units=${units}&APPID=${API_KEY}`);
  // console.log(data);
  return data;
};
...
// Weather City
const fetchWeatherForecastCity = async (cityName = 'Kyiv', units = 'metric') => {
