...
if (NewsLoad.filter.length=== 0 & NewsLoad.news.length=== 0 & NewsLoad.date.length=== 0) {
    ...
    if (NewsLoad.filter.length> 0 ) {
        if(NewsLoad.date.length> 0 ){
            ...
            NewsLoad.getFullNewsList().then(data => {
            ...
            NewsLoad.getSerchFilterList().then(data => {
                if (data.response.docs.length === 0) {
                  form.reset();
                  newsList.innerHTML =""
                    return createEmptyMarkup();
        ...
        if(NewsLoad.date.length> 0 ){
            ...
            NewsLoad.getSerchDateList().then(data => {
                if (data.response.docs.length === 0) {
                  form.reset();
                  newsList.innerHTML =""
                    return createEmptyMarkup();
            ...
            //запит по пошуку 
            NewsLoad.getSerchList().then(data => {
                if (data.response.docs.length === 0) {
                  form.reset();
                  newsList.innerHTML =""
                    return createEmptyMarkup();
    ...
    if (NewsLoad.filter.length> 0 ) {
        if(NewsLoad.date.length> 0 ){
            ...
            NewsLoad.getfilterdatelist().then(data => {
                if (data.response.docs.length === 0) {
                  form.reset();
                  newsList.innerHTML =""
                    return createEmptyMarkup();
          ...
          //запит по фільтру 
          NewsLoad.getfilterlist().then(data => {
            if (data.response.docs.length === 0) {
              form.reset();
              newsList.innerHTML =""
                return createEmptyMarkup();
        ...
        if(NewsLoad.date.length> 0 ){
            //запит по даті
            NewsLoad.getdatelist().then(data => {
                if (data.response.docs.length === 0) {
                  form.reset();
                  newsList.innerHTML =""
                    return createEmptyMarkup();
